	  ...
	  m_cl_platforms(NULL)
{
	...
	mApiChecker = new WebCLAPIBlocker(deviceName.UpperASCII());

	mOperationHandler = WTF::WrapUnique(new WebCLOperationHandler());
...
HeapVector<Member<WebCLPlatform>> WebCL::getPlatforms(ExceptionState& ec) {
	...
	for(WebCLPlatformMap::iterator iter = mClPlatformMap.begin(); iter != mClPlatformMap.end(); ++iter) {
		...
		mWebCLPlatformList.push_back(iter->value);
	}
	...
	if(createWebCLPlatformList()) {
		...
		for(WebCLPlatformMap::iterator iter = mClPlatformMap.begin(); iter != mClPlatformMap.end(); ++iter) {
			...
			for(j = 0; j < (int)platform_extensions.size(); j++) {
				...
				data.insert(platform_extensions[j]);
			}
		}

		for(data_iterator = data.begin(); data_iterator != data.end(); ++data_iterator) {
			supportedExtensionList.push_back(*data_iterator);
			CLLOG(INFO) << "supportedExtensionList.push_back(" << &data_iterator;
		}
	...
	for(WebCLPlatformMap::iterator iter = mClPlatformMap.begin(); iter != mClPlatformMap.end(); ++iter)
	{
		...
		if (platform->enableExtension(extensionNameLocal, ec)) {
			if (!enableExtensionList.Contains(extensionNameLocal)) {
				enableExtensionList.push_back(extensionNameLocal);
			}
			result = true;
		}
			...
			if (device->enableExtension(extensionNameLocal, ec)) {
				if (!enableExtensionList.Contains(extensionNameLocal)) {
					...
					if (extensionNameLocal == "KHR_fp16") {
						enableExtensionList.push_back("cl_khr_fp16");
					} else if(extensionNameLocal == "KHR_fp64") {
						enableExtensionList.push_back("cl_khr_fp64");
					}
				}
				result = true;
			}
...
WebCLContext* WebCL::createContext(unsigned int deviceType, ExceptionState& ec)
{
	...
	contextProperties.push_back((cl_context_properties)platform->getPlatformId());
	...
	if(mGL) {
		...
		if (enableGlSharing) {
			...
			contextProperties.push_back(CL_GL_CONTEXT_KHR);
			contextProperties.push_back((cl_context_properties)glContext);
			contextProperties.push_back(CL_EGL_DISPLAY_KHR);
			contextProperties.push_back((cl_context_properties)glDisplay);
		}
	}
		...
		for(size_t i=0; i<devices.size(); i++) {
			...
			for(size_t j=0; j<tmpList.size(); j++) {
				if(!enabledExtensionList.Contains(tmpList[j]))
					enabledExtensionList.push_back(tmpList[j]);
			}
		}
...
WebCLContext* WebCL::findCLContext(cl_obj_key key)
{
	...
	Member<WebCLContext> context = mClContextMap.at(key);
	return context.Get();
}
...
Member<WebCLPlatform> WebCL::findCLPlatform(cl_obj_key key, ExceptionState& ec)
{
	...
	if (mClPlatformMap.Contains(key)) {
		platform = mClPlatformMap.at(key);
	}
