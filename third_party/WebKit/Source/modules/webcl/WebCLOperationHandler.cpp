...
void WebCLOperationHandler::startHandling()
{
	if(!mIsShared) {
		...
		mSharedData = WTF::WrapUnique(new base::SharedMemory());
		mSharedData->CreateAndMapAnonymous(kMaxBufferSize);
#elif defined(OS_LINUX)
		// mSharedData = WTF::WrapUnique(content::RenderThread::Get()->HostAllocateSharedMemoryBuffer(kMaxBufferSize).release());
		mSharedData = WTF::WrapUnique(Platform::Current()->getSharedMemoryForWebCL(kMaxBufferSize));
		mSharedData->Map(kMaxBufferSize);
#endif
		dataHandle = mSharedData->handle().Duplicate();
		mSharedDataPtr = static_cast<char*>(mSharedData->memory());

#if defined(OS_ANDROID)
		mSharedOperation = WTF::WrapUnique(new base::SharedMemory());
		mSharedOperation->CreateAndMapAnonymous(sizeof(BaseOperationData));
#elif defined(OS_LINUX)
		// mSharedOperation = WTF::WrapUnique(content::RenderThread::Get()->HostAllocateSharedMemoryBuffer(sizeof(BaseOperationData)).release());
		 mSharedOperation = WTF::WrapUnique(Platform::Current()->getSharedMemoryForWebCL(sizeof(BaseOperationData)));
		mSharedOperation->Map(sizeof(BaseOperationData));
#endif
		operationHandle = mSharedOperation->handle().Duplicate();
		mSharedOperationPtr = static_cast<BaseOperationData*>(mSharedOperation->memory());

#if defined(OS_ANDROID)
		mSharedResult = WTF::WrapUnique(new base::SharedMemory());
		mSharedResult->CreateAndMapAnonymous(sizeof(BaseResultData));
#elif defined(OS_LINUX)
		// mSharedResult = WTF::WrapUnique(content::RenderThread::Get()->HostAllocateSharedMemoryBuffer(sizeof(BaseResultData)).release());
		mSharedResult = WTF::WrapUnique(Platform::Current()->getSharedMemoryForWebCL(sizeof(BaseResultData)));
		mSharedResult->Map(sizeof(BaseResultData));
#endif
		resultHandle = mSharedResult->handle().Duplicate();
		mSharedResultPtr = static_cast<BaseResultData*>(mSharedResult->memory());

#if defined(OS_ANDROID)
		mSharedEvents = WTF::WrapUnique(new base::SharedMemory());
		mSharedEvents->CreateAndMapAnonymous(kMaxEventSize);
#elif defined(OS_LINUX)
		// mSharedEvents = WTF::WrapUnique(content::RenderThread::Get()->HostAllocateSharedMemoryBuffer(kMaxEventSize).release());
		mSharedEvents = WTF::WrapUnique(Platform::Current()->getSharedMemoryForWebCL(kMaxEventSize));
		mSharedEvents->Map(kMaxEventSize);
#endif
		eventsHandle = mSharedEvents->handle().Duplicate();
