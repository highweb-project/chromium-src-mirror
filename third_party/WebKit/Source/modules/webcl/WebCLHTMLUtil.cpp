...
bool WebCLHTMLUtil::extractDataFromCanvas(HTMLCanvasElement* canvas, Vector<uint8_t>& data, size_t& canvasSize, ExceptionState& es)
{
    ...
    //     return false;
    // }
    if (!packImageData(canvas->copiedImage(BackBuffer, AccelerationHint::PreferAcceleration, SnapshotReasonGetCopiedImage).get(), WebGLImageConversion::HtmlDomCanvas, canvas->width(), canvas->height(), data)) {
      	es.throwDOMException(WebCLException::INVALID_HOST_PTR, "WebCLException::INVALID_HOST_PTR");
        return false;
    }
