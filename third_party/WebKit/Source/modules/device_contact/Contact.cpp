...
void Contact::findContactInternal(int requestId)
{
	...
	if (!contactManager.is_bound()) {
		Platform::current()->interfaceProvider()->getInterface(mojo::GetProxy(&contactManager));
	}
...
void Contact::OnContactResultsWithObject(int32_t requestId, uint32_t error, WTF::Optional<WTF::Vector<device::blink::ContactObjectPtr>> result) {
...
void Contact::OnContactResultsWithFindOption(int32_t requestId, uint32_t error, WTF::Optional<WTF::Vector<device::blink::ContactObjectPtr>> result) {
