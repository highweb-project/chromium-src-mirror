...
VKCResult WebVKCBuffer::readBuffer(VKCuint index, VKCuint bufferByteSize, NotShared<DOMArrayBufferView> hostPtr, ExceptionState& ec) {
	...
	if(result.result == VK_SUCCESS) {
		mDevice->getVKC()->getOperationResultData(hostPtr.View()->BaseAddress(), hostPtr.View()->byteLength());
	} else {
...
VKCResult WebVKCBuffer::writeBuffer(VKCuint index, VKCuint bufferByteSize, NotShared<DOMArrayBufferView> hostPtr, ExceptionState& ec) {
	if (index > mByteSize || (index + bufferByteSize) > mByteSize) {
		VKCLOG(INFO) << "index not valid";
		WebVKCException::throwVKCException(VKC_ARGUMENT_NOT_VALID, ec);
		return (VKCResult)VKC_ARGUMENT_NOT_VALID;
	...
	mDevice->getVKC()->setOperationData(hostPtr.View()->BaseAddress(), hostPtr.View()->byteLength());
