...
Messaging::Messaging(LocalFrame& frame)
	: ActiveScriptWrappable(this),
		SuspendableObject((ExecutionContext*)frame.document()),
	 	mIsListening(false)
{
...
void Messaging::sendMessage(Member<MessageObject> message)
{
	...
	if (!messageManager.is_bound()) {
		Platform::current()->interfaceProvider()->getInterface(mojo::GetProxy(&messageManager));
	}
...
void Messaging::findMessageResult(
	uint32_t requestID, uint32_t error, WTF::Optional<WTF::Vector<device::blink::MessageObjectPtr>> results) {
...
void Messaging::contextDestroyed()
{
