...
void DeviceSoundManagerLinux::outputDeviceType(outputDeviceTypeCallback callback) {
  std::string execResult = exec("pacmd list-sinks");
  mojom::DeviceSound_ResultCodePtr result(mojom::DeviceSound_ResultCode::New());
  if (execResult.empty()) {
    result->resultCode = int32_t(mojom::devicesound_ErrorCodeList::FAILURE);
    result->outputType = -1;
  } else {
    ...
    if (result->outputType == -1) {
      ...
      result->resultCode = int32_t(mojom::devicesound_ErrorCodeList::SUCCESS);
    }
  }
...
void DeviceSoundManagerLinux::deviceVolume(deviceVolumeCallback callback) {
  std::string execResult = exec("pacmd list-sinks");
  getDeviceVolume(execResult);
  mojom::DeviceSound_ResultCodePtr result(mojom::DeviceSound_ResultCode::New());
  result->volume = mojom::DeviceSound_Volume::New();
  result->functionCode = int32_t(mojom::devicesound_function::FUNC_DEVICE_VOLUME);
  if (execResult.empty()) {
    result->resultCode = int32_t(mojom::devicesound_ErrorCodeList::FAILURE);
  } else {
...
DeviceSoundManagerLinux::DeviceSoundManagerLinux(mojom::DeviceSoundManagerRequest request)
...
int32_t DeviceSoundManagerLinux::getOutputDeviceLinux(std::string& execResult) {
  ...
  if (selectedDeviceIndex != std::string::npos) {
    ...
    if (activePortIndex == std::string::npos) {
      ...
      return int32_t(mojom::device_type::DEVICE_DEFAULT);
    }
  } else {
