...
void InitializeStaticCLBindings(CLApi* apiImpl) {
	...
	std::string vulaknSoFile = "libvulkan.so";
...
// http://stackoverflow.com/questions/306533/how-do-i-get-a-list-of-files-in-a-directory-in-c
	...
	base::FilePath fileNameVulkan = base::FilePath(vulaknSoFile);
	...
	if (!vulkan_library) {
		DLOG(INFO) << "load vulkan library failed!!";
		// [ERROR:native_library_posix.cc(41)] dlclose failed: NULL library handle
		// base::UnloadNativeLibrary(opencl_library);
	}
	else {
		DLOG(INFO) << "load vulkan library success!!";
		apiImpl->InitVulkanApi(vulkan_library);
	}
...
VkResult CLApi::doVKCreateInstance(const VkInstanceCreateInfo* pCreateInfo, const VkAllocationCallbacks* pAllocator, VkInstance* pInstance) {
	CLLOG(INFO) << "CLApi::doCreateInstance Vulkan";
	VkResult err = vk_create_instance_(pCreateInfo, pAllocator, pInstance);
	CLLOG(INFO) << "vkCreateInstance err : " << err;
	return err;
}

VkResult CLApi::doVKEnumeratePhysicalDevices(VkInstance pCreateInfo, uint32_t* pPhysicalDeviceCount, VkPhysicalDevice* pPhysicalDevices) {
	CLLOG(INFO) << "CLApi::doVKEnumeratePhysicalDevices";
	VkResult err = vk_enumerate_physical_device_(pCreateInfo, pPhysicalDeviceCount, pPhysicalDevices);
	CLLOG(INFO) << "vkEnumeratePhysicalDevices : " << err;
	return err;
}

void CLApi::doVKGetPhysicalDeviceProperties(VkPhysicalDevice physicalDevice, VkPhysicalDeviceProperties* pProperties) {
	CLLOG(INFO) << "CLApi::doVKGetPhysicalDeviceProperties";
	vk_get_physical_device_properties_(physicalDevice, pProperties);
}

void CLApi::doVKGetPhysicalDeviceQueueFamilyProperties(VkPhysicalDevice physicalDevice, uint32_t* pQueueFamilyPropertyCount, VkQueueFamilyProperties* pQueueFamilyProperties) {
	CLLOG(INFO) << "CLApi::doVKGetPhysicalDeviceQueueFamilyProperties";
	vk_get_physical_device_queue_family_properties_(physicalDevice, pQueueFamilyPropertyCount, pQueueFamilyProperties);
}

VkResult CLApi::doVKCreateDevice(VkPhysicalDevice physicalDevice, const VkDeviceCreateInfo* pCreateInfo, const VkAllocationCallbacks* pAllocator, VkDevice* pDevice) {
	CLLOG(INFO) << "CLApi::doVKCreateDevice";
	VkResult err = vk_create_device_(physicalDevice, pCreateInfo, pAllocator, pDevice);
	CLLOG(INFO) << "vkCreateDevice : " << err << " " << pDevice;
	return err;
}

void CLApi::doVKGetDeviceQueue(VkDevice device, uint32_t queueFamilyIndex, uint32_t queueIndex, VkQueue* pQueue) {
	CLLOG(INFO) << "CLApi::doVKGetDeviceQueue " << pQueue;
	vk_get_device_queue_(device, queueFamilyIndex, queueIndex, pQueue);
	CLLOG(INFO) << "vkGetDeviceQueue : " << pQueue;
}

void CLApi::doVKGetPhysicalDeviceMemoryProperties(VkPhysicalDevice physicalDevice, VkPhysicalDeviceMemoryProperties* pMemoryProperties) {
	CLLOG(INFO) << "CLApi::doVKGetPhysicalDeviceMemoryProperties " << pMemoryProperties;
	vk_get_physical_device_memory_properties_(physicalDevice, pMemoryProperties);
	CLLOG(INFO) << "vkGetPhysicalDeviceMemoryProperties : " << pMemoryProperties;
}

VkResult CLApi::doVKCreateCommandPool(VkDevice device, const VkCommandPoolCreateInfo* pCreateInfo, const VkAllocationCallbacks* pAllocator, VkCommandPool* pCommandPool) {
	CLLOG(INFO) << "CLApi::doVKCreateCommandPool";
	VkResult err = vk_create_command_pool_(device, pCreateInfo, pAllocator, pCommandPool);
	CLLOG(INFO) << "vkCreateCommandPool : " << err;
	return err;
}

VkResult CLApi::doVKAllocateCommandBuffers(VkDevice device, const VkCommandBufferAllocateInfo* pAllocateInfo, VkCommandBuffer* pCommandBuffers) {
	CLLOG(INFO) << "CLApi::doVKAllocateCommandBuffers";
	VkResult err = vk_allocate_command_buffers_(device, pAllocateInfo, pCommandBuffers);
	CLLOG(INFO) << "vkAllocateCommandBuffers : " << err;
	return err;
}

VkResult CLApi::doVKCreateBuffer(VkDevice device, const VkBufferCreateInfo* pCreateInfo, const VkAllocationCallbacks* pAllocator, VkBuffer* pBuffer) {
	CLLOG(INFO) << "CLApi::doVKCreateBuffer";
	VkResult err = vk_create_buffer_(device, pCreateInfo, pAllocator, pBuffer);
	CLLOG(INFO) << "vkCreateBuffer : " << err;
	return err;
}

void CLApi::doVKGetBufferMemoryRequirements(VkDevice device, VkBuffer buffer, VkMemoryRequirements* pMemoryRequirements) {
	CLLOG(INFO) << "CLApi::doVKGetBufferMemoryRequirements " << pMemoryRequirements;
	vk_get_buffer_memory_requirements_(device, buffer, pMemoryRequirements);
	CLLOG(INFO) << "vkGetBufferMemoryRequirements : " << pMemoryRequirements;
}

VkResult CLApi::doVKAllocateMemory(VkDevice device, const VkMemoryAllocateInfo* pAllocateInfo, const VkAllocationCallbacks* pAllocator, VkDeviceMemory* pMemory) {
	CLLOG(INFO) << "CLApi::doVKAllocateMemory";
	VkResult err = vk_allocate_memory_(device, pAllocateInfo, pAllocator, pMemory);
	CLLOG(INFO) << "vkAllocateMemory : " << err;
	return err;
}

VkResult CLApi::doVKBindBufferMemory(VkDevice device, VkBuffer buffer, VkDeviceMemory memory, VkDeviceSize memoryOffset) {
	CLLOG(INFO) << "CLApi::doVKBindBufferMemory";
	VkResult err = vk_bind_buffer_memory_(device, buffer, memory, memoryOffset);
	CLLOG(INFO) << "vkBindBufferMemory : " << err;
	return err;
}

VkResult CLApi::doVKMapMemory(VkDevice device, VkDeviceMemory memory, VkDeviceSize offset, VkDeviceSize size, VkMemoryMapFlags flags, void** ppData)  {
	// CLLOG(INFO) << "CLApi::doVKMapMemory";
	VkResult err = vk_map_memory_(device, memory, offset, size, flags, ppData);
	// CLLOG(INFO) << "vkMapMemory : " << err;
	return err;
}

void CLApi::doVKUnmapMemory(VkDevice device, VkDeviceMemory memory) {
	// CLLOG(INFO) << "CLApi::doVKUnmapMemory";
	vk_unmap_memory_(device, memory);
}

VkResult CLApi::doVKCreateShaderModule(VkDevice device, const VkShaderModuleCreateInfo* pCreateInfo, const VkAllocationCallbacks* pAllocator, VkShaderModule* pShaderModule) {
	CLLOG(INFO) << "CLApi::doVKCreateShaderModule";
	VkResult err = vk_create_shader_module_(device, pCreateInfo, pAllocator, pShaderModule);
	CLLOG(INFO) << "vkCreateShaderModule : " << err;
	return err;
}

VkResult CLApi::doVKCreateDescriptorSetLayout(VkDevice device, const VkDescriptorSetLayoutCreateInfo* pCreateInfo, const VkAllocationCallbacks* pAllocator, VkDescriptorSetLayout* pSetLayout) {
	CLLOG(INFO) << "CLApi::doVKCreateDescriptorSetLayout";
	VkResult err = vk_create_descriptor_set_layout_(device, pCreateInfo, pAllocator, pSetLayout);
	CLLOG(INFO) << "vkCreateDescriptorSetLayout : " << err;
	return err;
}

VkResult CLApi::doVKCreatePipelineLayout(VkDevice device, const VkPipelineLayoutCreateInfo* pCreateInfo, const VkAllocationCallbacks* pAllocator, VkPipelineLayout* pPipelineLayout) {
	CLLOG(INFO) << "CLApi::doVKCreatePipelineLayout";
	VkResult err = vk_create_pipeline_layout_(device, pCreateInfo, pAllocator, pPipelineLayout);
	CLLOG(INFO) << "vkCreatePipelineLayout : " << err;
	return err;
}

VkResult CLApi::doVKCreatePipelineCache(VkDevice device, const VkPipelineCacheCreateInfo* pCreateInfo, const VkAllocationCallbacks* pAllocator, VkPipelineCache* pPipelineCache) {
	CLLOG(INFO) << "CLApi::doVKCreatePipelineCache";
	VkResult err = vk_create_pipeline_cache_(device, pCreateInfo, pAllocator, pPipelineCache);
	CLLOG(INFO) << "vkCreatePipelineCache : " << err;
	return err;
}

VkResult CLApi::doVKCreateComputePipelines(VkDevice device, VkPipelineCache pipelineCache, uint32_t createInfoCount, const VkComputePipelineCreateInfo* pCreateInfos, const VkAllocationCallbacks* pAllocator, VkPipeline* pPipelines) {
	CLLOG(INFO) << "CLApi::doVKCreateComputePipelines";
	VkResult err = vk_create_compute_pipelines_(device, pipelineCache, createInfoCount, pCreateInfos, pAllocator, pPipelines);
	CLLOG(INFO) << "vkCreateComputePipelines : " << err;
	return err;
}

VkResult CLApi::doVKCreateDescriptorPool(VkDevice device, const VkDescriptorPoolCreateInfo* pCreateInfo, const VkAllocationCallbacks* pAllocator, VkDescriptorPool* pDescriptorPool) {
	CLLOG(INFO) << "CLApi::doVKCreateDescriptorPool";
	VkResult err = vk_create_descriptor_pool_(device, pCreateInfo, pAllocator, pDescriptorPool);
	CLLOG(INFO) << "vkCreateDescriptorPool : " << err;
	return err;
}

VkResult CLApi::doVKAllocateDescriptorSets(VkDevice device, const VkDescriptorSetAllocateInfo* pAllocateInfo, VkDescriptorSet* pDescriptorSets) {
	CLLOG(INFO) << "CLApi::doVKAllocateDescriptorSets";
	VkResult err = vk_allocate_descriptor_sets_(device, pAllocateInfo, pDescriptorSets);
	CLLOG(INFO) << "vkAllocateDescriptorSets : " << err;
	return err;
}

void CLApi::doVKUpdateDescriptorSets(VkDevice device, uint32_t descriptorWriteCount, const VkWriteDescriptorSet* pDescriptorWrites, uint32_t descriptorCopyCount, const VkCopyDescriptorSet* pDescriptorCopies) {
	CLLOG(INFO) << "CLApi::doVKUpdateDescriptorSets " << pDescriptorCopies;
	vk_update_descriptor_sets_(device, descriptorWriteCount, pDescriptorWrites, descriptorCopyCount, pDescriptorCopies);
	CLLOG(INFO) << "vkUpdateDescriptorSets : " << pDescriptorCopies;
}

VkResult CLApi::doVKBeginCommandBuffer(VkCommandBuffer commandBuffer, const VkCommandBufferBeginInfo* pBeginInfo) {
	CLLOG(INFO) << "CLApi::doVKBeginCommandBuffer";
	VkResult err = vk_begin_command_buffer_(commandBuffer, pBeginInfo);
	CLLOG(INFO) << "vkBeginCommandBuffer : " << err;
	return err;
}

void CLApi::doVKCmdBindDescriptorSets(VkCommandBuffer commandBuffer, VkPipelineBindPoint pipelineBindPoint, VkPipelineLayout layout, uint32_t firstSet, uint32_t descriptorSetCount, const VkDescriptorSet* pDescriptorSets, uint32_t dynamicOffsetCount, const uint32_t* pDynamicOffsets) {
	CLLOG(INFO) << "CLApi::doVKCmdBindDescriptorSets";
	...
	CLLOG(INFO) << "CLApi::doVKCmdBindDescriptorSets finish";
}

void CLApi::doVKCmdBindPipeline(VkCommandBuffer commandBuffer, VkPipelineBindPoint pipelineBindPoint, VkPipeline pipeline) {
	CLLOG(INFO) << "CLApi::doVKCmdBindPipeline";
	vk_cmd_bind_pipeline_(commandBuffer, pipelineBindPoint, pipeline);
}

void CLApi::doVKCmdDispatch(VkCommandBuffer commandBuffer, uint32_t x, uint32_t y, uint32_t z) {
	// CLLOG(INFO) << "CLApi::doVKCmdDispatch";
	vk_cmd_dispatch_(commandBuffer, x, y, z);
}

void CLApi::doVKCmdPipelineBarrier(VkCommandBuffer commandBuffer, VkPipelineStageFlags srcStageMask, VkPipelineStageFlags dstStageMask, VkDependencyFlags dependencyFlags, uint32_t memoryBarrierCount, const VkMemoryBarrier* pMemoryBarriers, uint32_t bufferMemoryBarrierCount, const VkBufferMemoryBarrier* pBufferMemoryBarriers, uint32_t imageMemoryBarrierCount, const VkImageMemoryBarrier* pImageMemoryBarriers) {
	// CLLOG(INFO) << "CLApi::doVKCmdPipelineBarrier";
...
VkResult CLApi::doVKEndCommandBuffer(VkCommandBuffer commandBuffer) {
	CLLOG(INFO) << "CLApi::doVKEndCommandBuffer";
	VkResult err = vk_end_command_buffer_(commandBuffer);
	CLLOG(INFO) << "vkEndCommandBuffer : " << err;
	return err;
}

VkResult CLApi::doVKQueueSubmit(VkQueue queue, uint32_t submitCount, const VkSubmitInfo* pSubmits, VkFence fence) {
	// CLLOG(INFO) << "CLApi::doVKQueueSubmit";
	VkResult err = vk_queue_submit_(queue, submitCount, pSubmits, fence);
	// CLLOG(INFO) << "vkQueueSubmit : " << err;
	return err;
}

VkResult CLApi::doVKQueueWaitIdle(VkQueue queue) {
	// CLLOG(INFO) << "CLApi::doVKQueueWaitIdle";
	VkResult err = vk_queue_wait_idle_(queue);
	// CLLOG(INFO) << "vkQueueWaitIdle : " << err;
	return err;
}

VkResult CLApi::doVKDeviceWaitIdle(VkDevice device) {
	// CLLOG(INFO) << "CLApi::doVKDeviceWaitIdle";
	VkResult err = vk_device_wait_idle_(device);
	// CLLOG(INFO) << "vkDeviceWaitIdle : " << err;
	return err;	
}

void CLApi::doVKCmdCopyBuffer(VkCommandBuffer commandBuffer, VkBuffer srcBuffer, VkBuffer dstBuffer, uint32_t regionCount, const VkBufferCopy* pRegions) {
	CLLOG(INFO) << "CLApi::doVKCmdCopyBuffer";
	vk_cmd_copy_buffer_(commandBuffer, srcBuffer, dstBuffer, regionCount, pRegions);
}

void CLApi::doVKFreeMemory(VkDevice device, VkDeviceMemory memory, const VkAllocationCallbacks* pAllocator) {
	CLLOG(INFO) << "CLApi::doVKFreeMemory";
	vk_free_memory_(device, memory, pAllocator);
}

void CLApi::doVKDestroyBuffer(VkDevice device, VkBuffer buffer, const VkAllocationCallbacks* pAllocator) {
	CLLOG(INFO) << "CLApi::doVKDestroyBuffer";
	vk_destroy_buffer_(device, buffer, pAllocator);
}

VkResult CLApi::doVKFreeDescriptorSets(VkDevice device, VkDescriptorPool descriptorPool, uint32_t descriptorSetCount, const VkDescriptorSet* pDescriptorSets) {
	CLLOG(INFO) << "CLApi::doVKFreeDescriptorSets";
	VkResult err = vk_free_descriptor_sets_(device, descriptorPool, descriptorSetCount, pDescriptorSets);
	CLLOG(INFO) << "vkFreeDescriptorSets : " << err;
	return err;
}

void CLApi::doVKDestroyDescriptorPool(VkDevice device, VkDescriptorPool descriptorPool, const VkAllocationCallbacks* pAllocator) {
	CLLOG(INFO) << "CLApi::doVKDestroyDescriptorPool";
	vk_destroy_descriptor_pool_(device, descriptorPool, pAllocator);
}

void CLApi::doVKFreeCommandBuffers(VkDevice device, VkCommandPool commandPool, uint32_t commandBufferCount, const VkCommandBuffer* pCommandBuffers) {
	CLLOG(INFO) << "CLApi::doVKFreeCommandBuffers";
	vk_free_command_buffers_(device, commandPool, commandBufferCount, pCommandBuffers);
}

void CLApi::doVKDestroyCommandPool(VkDevice device, VkCommandPool commandPool, const VkAllocationCallbacks* pAllocator) {
	CLLOG(INFO) << "CLApi::doVKDestroyCommandPool";
	vk_destroy_command_pool_(device, commandPool, pAllocator);
}

void CLApi::doVKDestroyDevice(VkDevice device, const VkAllocationCallbacks* pAllocator) {
	CLLOG(INFO) << "CLApi::doVKDestroyDevice";
...
void CLApi::InitVulkanApi(base::NativeLibrary vulkanLib) {
	DLOG(INFO) << "CLApi::InitVulkanApi";
	vulkan_library_ = vulkanLib;
	vk_create_instance_ = CL_API_LOAD(vulkan_library_, "vkCreateInstance", VKCreateInstance);
	vk_enumerate_physical_device_ = CL_API_LOAD(vulkan_library_, "vkEnumeratePhysicalDevices", VKEnumeratePhysicalDevices);
	vk_get_physical_device_properties_ = CL_API_LOAD(vulkan_library_, "vkGetPhysicalDeviceProperties", VKGetPhysicalDeviceProperties);
	vk_get_physical_device_queue_family_properties_ = CL_API_LOAD(vulkan_library_, "vkGetPhysicalDeviceQueueFamilyProperties", VKGetPhysicalDeviceQueueFamilyProperties);
	vk_create_device_ = CL_API_LOAD(vulkan_library_, "vkCreateDevice", VKCreateDevice);
	vk_get_device_queue_ = CL_API_LOAD(vulkan_library_, "vkGetDeviceQueue", VKGetDeviceQueue);
	vk_get_physical_device_memory_properties_ = CL_API_LOAD(vulkan_library_, "vkGetPhysicalDeviceMemoryProperties", VKGetPhysicalDeviceMemoryProperties);
	vk_create_command_pool_ = CL_API_LOAD(vulkan_library_, "vkCreateCommandPool", VKCreateCommandPool);
	vk_allocate_command_buffers_ = CL_API_LOAD(vulkan_library_, "vkAllocateCommandBuffers", VKAllocateCommandBuffers);
	vk_create_buffer_ = CL_API_LOAD(vulkan_library_, "vkCreateBuffer", VKCreateBuffer);
	vk_get_buffer_memory_requirements_ = CL_API_LOAD(vulkan_library_, "vkGetBufferMemoryRequirements", VKGetBufferMemoryRequirements);
	vk_allocate_memory_ = CL_API_LOAD(vulkan_library_, "vkAllocateMemory", VKAllocateMemory);
	vk_bind_buffer_memory_ = CL_API_LOAD(vulkan_library_, "vkBindBufferMemory", VKBindBufferMemory);
	vk_map_memory_ = CL_API_LOAD(vulkan_library_, "vkMapMemory", VKMapMemory);
	vk_unmap_memory_ = CL_API_LOAD(vulkan_library_, "vkUnmapMemory", VKUnmapMemory);
	vk_create_shader_module_ = CL_API_LOAD(vulkan_library_, "vkCreateShaderModule", VKCreateShaderModule);
	vk_create_descriptor_set_layout_ = CL_API_LOAD(vulkan_library_, "vkCreateDescriptorSetLayout", VKCreateDescriptorSetLayout);
	vk_create_pipeline_layout_ = CL_API_LOAD(vulkan_library_, "vkCreatePipelineLayout", VKCreatePipelineLayout);
	vk_create_pipeline_cache_ = CL_API_LOAD(vulkan_library_, "vkCreatePipelineCache", VKCreatePipelineCache);
	vk_create_compute_pipelines_ = CL_API_LOAD(vulkan_library_, "vkCreateComputePipelines", VKCreateComputePipelines);
	vk_create_descriptor_pool_ = CL_API_LOAD(vulkan_library_, "vkCreateDescriptorPool", VKCreateDescriptorPool);
	vk_allocate_descriptor_sets_ = CL_API_LOAD(vulkan_library_, "vkAllocateDescriptorSets", VKAllocateDescriptorSets);
	vk_update_descriptor_sets_ = CL_API_LOAD(vulkan_library_, "vkUpdateDescriptorSets", VKUpdateDescriptorSets);
	vk_begin_command_buffer_ = CL_API_LOAD(vulkan_library_, "vkBeginCommandBuffer", VKBeginCommandBuffer);
	vk_cmd_bind_pipeline_ = CL_API_LOAD(vulkan_library_, "vkCmdBindPipeline", VKCmdBindPipeline);
	vk_cmd_bind_descriptor_sets_ = CL_API_LOAD(vulkan_library_, "vkCmdBindDescriptorSets", VKCmdBindDescriptorSets);
	vk_cmd_dispatch_ = CL_API_LOAD(vulkan_library_, "vkCmdDispatch", VKCmdDispatch);
	vk_cmd_pipeline_barrier_ = CL_API_LOAD(vulkan_library_, "vkCmdPipelineBarrier", VKCmdPipelineBarrier);
	vk_end_command_buffer_ = CL_API_LOAD(vulkan_library_, "vkEndCommandBuffer", VKEndCommandBuffer);
	vk_queue_submit_ = CL_API_LOAD(vulkan_library_, "vkQueueSubmit", VKQueueSubmit);
	vk_queue_wait_idle_ = CL_API_LOAD(vulkan_library_, "vkQueueWaitIdle", VKQueueWaitIdle);
	vk_device_wait_idle_ = CL_API_LOAD(vulkan_library_, "vkDeviceWaitIdle", VKDeviceWaitIdle);
	vk_cmd_copy_buffer_ = CL_API_LOAD(vulkan_library_, "vkCmdCopyBuffer", VKCmdCopyBuffer);
	vk_free_memory_ = CL_API_LOAD(vulkan_library_, "vkFreeMemory", VKFreeMemory);
	vk_destroy_buffer_ = CL_API_LOAD(vulkan_library_, "vkDestroyBuffer", VKDestroyBuffer);
	vk_free_descriptor_sets_ = CL_API_LOAD(vulkan_library_, "vkFreeDescriptorSets", VKFreeDescriptorSets);
	vk_destroy_descriptor_pool_ = CL_API_LOAD(vulkan_library_, "vkDestroyDescriptorPool", VKDestroyDescriptorPool);
	vk_free_command_buffers_ = CL_API_LOAD(vulkan_library_, "vkFreeCommandBuffers", VKFreeCommandBuffers);
	vk_destroy_command_pool_ = CL_API_LOAD(vulkan_library_, "vkDestroyCommandPool", VKDestroyCommandPool);
	vk_destroy_device_ = CL_API_LOAD(vulkan_library_, "vkDestroyDevice", VKDestroyDevice);
}

void CLApi::initNBody(const char* filePath) {
	...
	// inst_info.enabledExtensionCount = (uint32_t)enabledExtensions.size();
	...
	// setupDebugging(instance, VK_DEBUG_REPORT_ERROR_BIT_EXT | VK_DEBUG_REPORT_WARNING_BIT_EXT, NULL);

	uint32_t gpu_count;
	err = doVKEnumeratePhysicalDevices(instance, &gpu_count, NULL);

	VkPhysicalDevice* pDeviceVector = new VkPhysicalDevice[gpu_count];
	CLLOG(INFO) << "vkEnumeratePhysicalDevices : " << err << ", " << gpu_count << ", " << pDeviceVector[0];

	err = doVKEnumeratePhysicalDevices(instance, &gpu_count, pDeviceVector);
	CLLOG(INFO) << "vkEnumeratePhsicalDevice : " << err << ", " << gpu_count << ", " << pDeviceVector[0];

	uint32_t queueIndex = 0;
	uint32_t queueCount;
	doVKGetPhysicalDeviceQueueFamilyProperties(pDeviceVector[0], &queueCount, NULL);

	std::vector<VkQueueFamilyProperties> queueProps;
	queueProps.resize(queueCount);
	doVKGetPhysicalDeviceQueueFamilyProperties(pDeviceVector[0], &queueCount, queueProps.data());
	...
	CLLOG(INFO) << "VKDeviceQueueCreateInfo queueFamilyIndex : " << queueIndex;
	...
	err = doVKCreateDevice(pDeviceVector[0], &deviceCreateInfo, nullptr, &vkDevice);

	CLLOG(INFO) << "createDevice : " << err;

	VkPhysicalDeviceMemoryProperties dMemoryProperties;
	doVKGetPhysicalDeviceMemoryProperties(pDeviceVector[0], &dMemoryProperties);

	
	CLLOG(INFO) << "queue : " << vkQueue;
	doVKGetDeviceQueue(vkDevice, queueIndex, 0, &vkQueue);
	CLLOG(INFO) << "vkGetDeviceQueue : " << vkQueue;
	...
	err = doVKCreateCommandPool(vkDevice, &cmdPoolInfo, nullptr, &cmdPool);
	CLLOG(INFO) << "vkCommandPool : " << err << ", " << cmdPool;
	...
	err = doVKAllocateCommandBuffers(vkDevice, &cmdBufAllocateInfo, &vkComputeCmdBuffer);
	CLLOG(INFO) << "vkAllocateCommandBuffers : " << err << ", " << vkComputeCmdBuffer;

	VkDescriptorBufferInfo descriptorBufferInfo[8];
	...
	for(int i = 0; i < bufferCount; i++) {
		...
		vkBufferInfo[i].sType = VK_STRUCTURE_TYPE_BUFFER_CREATE_INFO;
		vkBufferInfo[i].usage = VK_BUFFER_USAGE_STORAGE_BUFFER_BIT | VK_BUFFER_USAGE_TRANSFER_DST_BIT;
		vkBufferInfo[i].flags = 0;

		if (i < 2 || i > 4) {
			vkBufferInfo[i].size = sizeof(float) * BUFFER_SIZE;
		} else if (i == 2 || i == 4) {
			vkBufferInfo[i].size = sizeof(int);
		} else if (i == 3) {
			vkBufferInfo[i].size = sizeof(float);
		} else if( i == 5) {
			vkBufferInfo[i].size = BUFFER_SIZE * sizeof(float);
		}
		err = doVKCreateBuffer(vkDevice, &vkBufferInfo[i], nullptr, &vkComputeBuffer[i]);	
		
		VkMemoryRequirements memoryRequirements;
		doVKGetBufferMemoryRequirements(vkDevice, vkComputeBuffer[i], &memoryRequirements);
		CLLOG(INFO) << "vkGetBufferMemoryRequirements : " << memoryRequirements.size << ", " << memoryRequirements.alignment << ", " << memoryRequirements.memoryTypeBits;
		...
		for (uint32_t i = 0; i < VK_MAX_MEMORY_TYPES; i++)
		{
			if ((typeBits & 1) == 1)
			{
				if ((dMemoryProperties.memoryTypes[i].propertyFlags & VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT) == VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT)
				{
		...
		err = doVKAllocateMemory(vkDevice, &allocateInfo, nullptr, &vkDeviceMemory[i]);
		void* deviceMemoryPtr = nullptr;
		err = doVKMapMemory(vkDevice, vkDeviceMemory[i], 0, vkBufferInfo[i].size, 0, &deviceMemoryPtr);
		CLLOG(INFO) << "vkMapMemory : " << err << ", " << deviceMemoryPtr;

		if (i < 2 || i > 5) {
			float* testData = new float[BUFFER_SIZE] {0};
			memcpy(deviceMemoryPtr, testData, sizeof(float)* BUFFER_SIZE);

			doVKUnmapMemory(vkDevice, vkDeviceMemory[i]);
			...
			int data = 4096;
			memcpy(deviceMemoryPtr, &data, sizeof(int));
			doVKUnmapMemory(vkDevice, vkDeviceMemory[i]);
		} else if (i == 3) {
			float data = 0.005f;
			memcpy(deviceMemoryPtr, &data, sizeof(float));
			doVKUnmapMemory(vkDevice, vkDeviceMemory[i]);
		} else if (i == 4) {
			int data = 50;
			memcpy(deviceMemoryPtr, &data, sizeof(int));
			...
			float* testData = new float[BUFFER_SIZE * sizeof(float)] { 0 };
			memcpy(deviceMemoryPtr, testData, sizeof(float)* BUFFER_SIZE);

			doVKUnmapMemory(vkDevice, vkDeviceMemory[i]);
		}

		err = doVKBindBufferMemory(vkDevice, vkComputeBuffer[i], vkDeviceMemory[i], 0);
		...
		setLayoutBindings.push_back(layout);
	...
	err = doVKCreateDescriptorSetLayout(vkDevice, &descriptorLayout, nullptr, &computeDescriptorSetLayout);
	CLLOG(INFO) << "createDescriptorSetLayout : " << err << ", " << setLayoutBindings.size() << ", " << computeDescriptorSetLayout;
	
	CLLOG(INFO) << "poolSize : " << poolSize.size();
	...
	err = doVKCreateDescriptorPool(vkDevice, &descriptorPoolInfo, nullptr, &descriptorPool);
	CLLOG(INFO) << "createDescriptorPool : " << err << ", " << descriptorPool;
	...
	err = doVKAllocateDescriptorSets(vkDevice, &allocInfo, &computeDescriptorSet);
	CLLOG(INFO) << "allocateDescriptorSets : " << err << ", " << computeDescriptorSet;
	...
	for(int i = 0; i < bufferCount; i++) {
		...
		writeSet.pNext = NULL;
		writeSet.dstSet = computeDescriptorSet;
		...
		descriptorBufferInfo[i].buffer = vkComputeBuffer[i];
		descriptorBufferInfo[i].offset = 0;
		descriptorBufferInfo[i].range = vkBufferInfo[i].size;
		writeSet.pBufferInfo = &descriptorBufferInfo[i];
		computeWriteDescriptorSets.push_back(writeSet);
	}

	doVKUpdateDescriptorSets(vkDevice, computeWriteDescriptorSets.size(), computeWriteDescriptorSets.data(), 0, NULL);

	CLLOG(INFO) << "updateDescriptorSets";
	...
	err = doVKCreatePipelineLayout(vkDevice, &pipelineLayoutCreateInfo, nullptr, &computePipelineLayout);
	CLLOG(INFO) << "createPipelineLayout : " << err << ", " << computePipelineLayout;
	...
	char* sourceData = (char*)malloc(65535);
	size_t size = getShaderCodeFromUrl(std::string(filePath), sourceData, 65535);
	...
	moduleCreateInfo.pCode = (uint32_t*)sourceData;
	moduleCreateInfo.flags = 0;
	err = doVKCreateShaderModule(vkDevice, &moduleCreateInfo, NULL, &shaderModule);
	...
	err = doVKCreatePipelineCache(vkDevice, &pipelineCacheCreateInfo, nullptr, &pipelineCache);
	CLLOG(INFO) << "vkCreatePipelineCache : " << err << ", " << pipelineCache;
	VkPipeline pipeline;
	err = doVKCreateComputePipelines(vkDevice, pipelineCache, 1, &computePipelineCreateInfo, nullptr, &pipeline);
	CLLOG(INFO) << "crateComputePipelines : " << err << ", " << pipeline;
	if (err != VK_SUCCESS) {
		CLLOG(INFO) << "Fail createComputePipeline";
		return;
	}
	...
	err = doVKBeginCommandBuffer(vkComputeCmdBuffer, &cmdBufInfo);
	CLLOG(INFO) << "VkCommandBufferBeginInfo : " << err;

	doVKCmdBindPipeline(vkComputeCmdBuffer, VK_PIPELINE_BIND_POINT_COMPUTE, pipeline);
	doVKCmdBindDescriptorSets(vkComputeCmdBuffer, VK_PIPELINE_BIND_POINT_COMPUTE, computePipelineLayout, 0, 1, &computeDescriptorSet, 0, 0);

	doVKCmdDispatch(vkComputeCmdBuffer, (BUFFER_SIZE / (1024 * 4)), 1, 1);
	doVKCmdPipelineBarrier(vkComputeCmdBuffer, VK_PIPELINE_STAGE_COMPUTE_SHADER_BIT, VK_PIPELINE_STAGE_COMPUTE_SHADER_BIT, 0, 0, nullptr, 0, nullptr, 0, nullptr);
	VkBufferCopy copyRegion = {};
	copyRegion.size = BUFFER_SIZE * sizeof(float);
	doVKCmdCopyBuffer(vkComputeCmdBuffer, vkComputeBuffer[6], vkComputeBuffer[0], 1, &copyRegion);
	doVKCmdCopyBuffer(vkComputeCmdBuffer, vkComputeBuffer[7], vkComputeBuffer[1], 1, &copyRegion);

	err = doVKEndCommandBuffer(vkComputeCmdBuffer);
	CLLOG(INFO) << "vkEndCommandBuffer : " << err;

	err = doVKDeviceWaitIdle(vkDevice);
	CLLOG(INFO) << "vkDeviceWaitIdle : " << err;
...
void CLApi::doNBody() {
	...
	computeSubmitInfo.pCommandBuffers = &vkComputeCmdBuffer;
	...
	VkResult err;
	err = doVKQueueSubmit(vkQueue, 1, &computeSubmitInfo, VK_NULL_HANDLE);
	CLLOG(INFO) << "VkQueueSubmit : " << err;
	err = doVKQueueWaitIdle(vkQueue);
	CLLOG(INFO) << "VkQueueWaitIdle : " << err;
...
void CLApi::doReadVulkanBuffer() {
	...
	err = doVKMapMemory(vkDevice, vkDeviceMemory[index], 0, size, 0, &dataPtr);
	...
	mSharedResultPtr->int_01 = 0;

	doVKUnmapMemory(vkDevice, vkDeviceMemory[index]);

	CLLOG(INFO) << ">>result= "<< mSharedResultPtr->int_01 << ", " << index << ", " << size;
}

void CLApi::doWriteVulkanBuffer() {
	VkResult err;
	void* dataPtr;
	unsigned index = mSharedOperationPtr->uint_01;
	unsigned size = mSharedOperationPtr->uint_02;
...
//return value is sourceData's length
//if sourceData length > maxSourceSize, sourceData not full source and return 0.
unsigned CLApi::getShaderCodeFromUrl(std::string address, char* sourceData, unsigned maxSourceSize) {

	int sockfd, c;
	struct sockaddr_in addr;
	
	size_t protocolIndex = address.find("://");
	CLLOG(INFO) << "protocolIndex : " << protocolIndex;
	if (protocolIndex != std::string::npos) {
		address = address.substr(protocolIndex + 3);
		CLLOG(INFO) << "address : " << address;
	} else {
		CLLOG(INFO) << "not found protocol index";
	}

	size_t hostNameIndex = address.find("/");
	CLLOG(INFO) << "hostNameIndex : " << hostNameIndex;
	std::string hostName;
	std::string path;
	...
	CLLOG(INFO) << "hostName : " << hostName;
	size_t portIndex = hostName.find(":");
	int port = 80; 
	if (portIndex == std::string::npos) {
		CLLOG(INFO) << "hostname not has port number";
	} else {
		CLLOG(INFO) << "portIndex : " << portIndex << ", port : " << hostName.substr(portIndex + 1);
		port = stoi(hostName.substr(portIndex + 1));
		hostName = hostName.substr(0, portIndex);
	}
	CLLOG(INFO) << "hostName : " << hostName << ", port : " << port;
	CLLOG(INFO) << "path : " << path;

	struct hostent *host = gethostbyname(hostName.c_str());

	if (host == nullptr) {
		CLLOG(INFO) << "wrong url " << h_errno;
		return 0;
	}
	...
	while(r > 0) // I'll work on dynamically setting the size later on.
	{
		...
		memcpy(&sourceData[index], buffer, r);
		index += r;
		if (index >= maxSourceSize) {
			index = 0;
			break;
		}
	}
