...
DeviceStorageManagerLinux::DeviceStorageManagerLinux(mojo::InterfaceRequest<DeviceStorageManager> request)
...
void DeviceStorageManagerLinux::getDeviceStorage(const getDeviceStorageCallback& callback) {
  ...
  if (!execResult.empty()) {
    ...
    if (data.get() && data.get()->GetType() == base::Value::TYPE_DICTIONARY) {
      ...
      if (blockdevicesValue && blockdevicesValue->size() > 0) {
        ...
        if (deviceList && deviceList->GetSize() > 0) {
          for(size_t listIndex = 0; listIndex < deviceList->GetSize(); listIndex++) {
            DeviceStorage_StorageInfoPtr info(DeviceStorage_StorageInfo::New());
            ...
            if (info->isRemovable) {
              info->type = uint64_t(device::device_storage_type::DEVICE_USB);
            } else {
              info->type = uint64_t(device::device_storage_type::DEVICE_HARDDISK);
            }
  ...
  result->resultCode = int32_t(device::device_storage_ErrorCodeList::SUCCESS);
