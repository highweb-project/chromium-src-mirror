...
class CalendarManagerEmptyImpl : public CalendarManager {
 public:
  ...
  explicit CalendarManagerEmptyImpl(CalendarManagerRequest request)
  ...
  void CalendarDeviceApiFindEvent(
    ...
    const std::string& endAfter, bool mutiple, const CalendarDeviceApiFindEventCallback& callback) override {
  ...
  void CalendarDeviceApiDeleteEvent(const std::string& id, const CalendarDeviceApiDeleteEventCallback& callback) override {
    Calendar_ResultCodePtr result(Calendar_ResultCode::New());
    result->resultCode = NOT_SUPPORT_API;
 ...
 private:
  ...
  mojo::StrongBindingPtr<CalendarManager> binding_;
};

}  // namespace

// static
void CalendarManagerImpl::Create(CalendarManagerRequest request) {
