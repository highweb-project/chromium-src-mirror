...
VKCResult webvkc_createShaderModuleWithUrl(GpuChannelHost* webvkc_channel_, VKCPoint& vkcDevice, std::string& shaderPath, VKCPoint* vkcShaderModule) {
  return webvkc_channel_->webvkc_createShaderModuleWithUrl(vkcDevice, shaderPath, vkcShaderModule);
}
VKCResult GpuChannelHost::webvkc_createShaderModuleWithUrl(VKCPoint& vkcDevice, std::string& shaderPath, VKCPoint* vkcShaderModule) {
  VKCLOG(INFO) << "webvkc_createShaderModuleWithUrl : " << vkcDevice << ", " << shaderPath;
  int result_inter = (int)VKC_SEND_IPC_MESSAGE_FAILURE;

  if (!Send(new VulkanComputeChannelMsg_CreateShaderModuleWithUrl(vkcDevice, shaderPath, vkcShaderModule, &result_inter))) {
    return (VKCResult)VKC_SEND_IPC_MESSAGE_FAILURE;
  }
...
VKCResult webvkc_createShaderModuleWithSource(GpuChannelHost* webvkc_channel_, VKCPoint& vkcDevice, std::string& shaderCode, VKCPoint* vkcShaderModule) {
  return webvkc_channel_->webvkc_createShaderModuleWithSource(vkcDevice, shaderCode, vkcShaderModule);
}
VKCResult GpuChannelHost::webvkc_createShaderModuleWithSource(VKCPoint& vkcDevice, std::string& shaderCode, VKCPoint* vkcShaderModule) {
  VKCLOG(INFO) << "webvkc_createShaderModuleWithSource : " << vkcDevice;
  int result_inter = (int)VKC_SEND_IPC_MESSAGE_FAILURE;

  if (!Send(new VulkanComputeChannelMsg_CreateShaderModuleWithSource(vkcDevice, shaderCode, vkcShaderModule, &result_inter))) {
    return (VKCResult)VKC_SEND_IPC_MESSAGE_FAILURE;
  }
