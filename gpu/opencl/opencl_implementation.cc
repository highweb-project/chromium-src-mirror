		...
		void *user_data)
{
	...
	((cl_point*)user_data)[4] = reinterpret_cast<cl_point>(gpu_proxy_);
...
{
	gpu::CLGpuManagerProxy* parent_proxy = reinterpret_cast<gpu::CLGpuManagerProxy*>(((cl_point*)userData)[4]);
	CLLOG(INFO) << "CLApi::WebCLCallbackPtr, parentGpuChannel : " << parent_proxy;
	if(parent_proxy != nullptr) {
		...
		parent_proxy->Send((int)keyArr[2], keyArr[0], keyArr[1], keyArr[3]);
	}
	else
...
void CLApi::WebCLCallbackPtrProgram(cl_program program, void* userData)
{
	...
	gpu::CLGpuManagerProxy* parent_proxy = reinterpret_cast<gpu::CLGpuManagerProxy*>(((cl_point*)userData)[4]);
	CLLOG(INFO) << "CLApi::WebCLCallbackPtrProgram, parent_proxy : " << parent_proxy;
	if(parent_proxy != nullptr) {
		cl_point* keyArr = (cl_point*) userData;

		CLLOG(INFO) << "WebCLCallbackPtrProgram routeId=" << keyArr[2] << ", event key=" << keyArr[0] << ", callback key=" << keyArr[1] << ", object_type=" << keyArr[3];
		parent_proxy->Send(keyArr[2], keyArr[0], keyArr[1], keyArr[3]);
	}
	else
		CLLOG(INFO) << "can't find GpuChannel ptr.";
}
				...
				void* user_data)
{
	...
	if (((cl_point*)user_data)[2] == 0) {
		...
		CLLOG(INFO) << "handler id not null " << ((cl_point*)user_data)[2];
		CLLOG(INFO) << "nDebug, CLApi::doClBuildProgram, CLApi : " << this << ", gpu_proxy_ : " << gpu_proxy_;
		((cl_point*)user_data)[4] = (cl_point)gpu_proxy_;
